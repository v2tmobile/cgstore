...
function cgs_add_like(){
   ...
   if($data && $data['UID'] == $current_user->ID){
       ...
       $result = array('success'=>true,'total_like'=>$total_like,'onclick'=>'ats_on_unlike(\''.$key.'\',\''.$data['PID'].'\',this)');
    }else{
       ...
       if(in_array($data['PID'], $pids)){
          foreach ($pids as $key => $like) {
             if($like == $data['PID']) {
               ...
               $result = array('success'=>true,'total_like'=>$total_like,'onclick'=>'ats_on_like(\''.$key.'\',\''.$data['PID'].'\',this)');
               break;
            }
          }
     }else{
