<?php
class CGS_Shortcode extends WC_Shortcodes{
  public static function init() {
    add_shortcode( 'custom_category_products', __CLASS__ . '::custom_category_products' );
 }

 public function custom_category_products( $atts ) {
   $atts = shortcode_atts( array(
			'per_page' => '12',
			'author'=> '',
			'columns'  => '4',
			'orderby'  => 'date',
			'order'    => 'desc',
			'free'      =>false,
			'category' => '',  // Slugs
			'operator' => 'IN' // Possible values are 'IN', 'NOT IN', 'AND'.
		), $atts, 'custom_category_products' );

		$query_args = array(
			'post_type'           => 'product',
			'post_status'         => 'publish',
			//'ignore_sticky_posts' => 1,
			'posts_per_page'      => $atts['per_page'],
			'orderby'             => $atts['orderby'],
			'order'               => $atts['order'],
			//'meta_query'          => WC()->query->get_meta_query()
		);
		if($atts['author']) $query_args['author'] = $atts['author'];
		if($atts['free']) $query_args['meta_query'] = array(
			'key' => '_regular_price',
                  'value' => 0,
                  'compare' => '=',
                  'type' => 'NUMERIC'
                );
     if($atts['category']){
		$query_args = self::_maybe_add_category_args( $query_args, $atts['category'], $atts['operator'] );
	 }
    
	return self::product_loop( $query_args, $atts, 'custom_category_products' );
  }

  private static function product_loop( $query_args, $atts, $loop_name ) {
		global $woocommerce_loop;

		$products                    = new WP_Query( apply_filters( 'woocommerce_shortcode_products_query', $query_args, $atts, $loop_name ) );
		$columns                     = absint( $atts['columns'] );
		$woocommerce_loop['columns'] = $columns;
		$woocommerce_loop['name']    = $loop_name;

		ob_start();

		if ( $products->have_posts() ) {
			?>

			<?php do_action( "woocommerce_shortcode_before_{$loop_name}_loop" ); ?>

			<?php woocommerce_product_loop_start(); ?>

				<?php while ( $products->have_posts() ) : $products->the_post(); ?>

					<?php wc_get_template_part( 'content', 'product' ); ?>

				<?php endwhile; // end of the loop. ?>

			<?php woocommerce_product_loop_end(); ?>

			<?php do_action( "woocommerce_shortcode_after_{$loop_name}_loop" ); ?>

			<?php
		} else {
			do_action( "woocommerce_shortcode_{$loop_name}_loop_no_results" );
		}

		woocommerce_reset_loop();
		wp_reset_postdata();

		return '<div class="woocommerce columns-' . $columns . '">' . ob_get_clean() . '</div>';
	}

  private static function _maybe_add_category_args( $args, $category, $operator ) {
		if ( ! empty( $category ) ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'product_cat',
					'terms'    => array_map( 'sanitize_title', explode( ',', $category ) ),
					'field'    => 'slug',
					'operator' => $operator
				)
			);
		}

		return $args;
	}
}

add_action( 'init', array( 'CGS_Shortcode', 'init' ) );

 ?>